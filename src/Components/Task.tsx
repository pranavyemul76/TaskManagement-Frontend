...
const Task: React.FC<TaskProps> = ({ task }) => {
  ...
  return<>  <tr className="hover:bg-gray-50 border">
  <th className="flex gap-3 px-6 py-4 font-normal items-center text-gray-900">
   
    <div className="text-lg ml-3">
      <div className="font-medium text-gray-700" > <span>&#8226;</span> {task.Title}</div>
    </div>
  </th>
  <td  className="px-6 py-4 border hidden md:table-cell">
   <select value={task.Status}  onChange={handleChange} className={`w-full p-3 border-none  outline-none ${task.Status==="Completed"? 'bg-green-300':"bg-red-300" } `}>
      <option className="bg-white" value={'Completed'} >Completed</option>
      <option className="bg-white" value={'Pending'}>Pending</option>
      <option className="bg-white" value={'InProgress'}>Inprogress</option>
   </select> 
  </td>
  <td className="px-6 py-4 border hidden md:table-cell">{task.AssignTo}</td>
...
{show&& <TaskCreateOrUpdate Show={show} SetShow={HandleClose} InputData={task}></TaskCreateOrUpdate>}
{Open && <WarningModal id={task.Id} open={Open} SetOpen={HandleDeleteClose}></WarningModal>}
</>
}
